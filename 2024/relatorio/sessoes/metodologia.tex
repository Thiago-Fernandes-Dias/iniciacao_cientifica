...
Todos os experimentos foram executados cinco vezes, sendo cada execução com uma semente pseudoaleatório diferente. Os resultados reportados neste relatório são as médias dos resultados obtidos após as cinco execuções.
...
Os valores padrão são \texttt{`kernel'}: \texttt{`rbf'}; \texttt{`degree'}: 3; \texttt{`gamma'}: `scale'; \texttt{`coef0'}: 0; \texttt{`cache\_size'}: 200.
...
Os valores padrão dos hiperparâmetros são \texttt{`n\_estimators'}: 100; \texttt{`criterion'}: \texttt{`gini'}; \texttt{`max\_features'}: \texttt{`sqrt'}; \texttt{`bootstrap'}: \texttt{True}; \texttt{`warm\_start'}: \texttt{False}.
