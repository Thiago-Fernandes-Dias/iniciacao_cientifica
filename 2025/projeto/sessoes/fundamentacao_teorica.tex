...
\subsection{Extração de características de digitais}\label{subsec:extracao_de_caracteristicas}
...
O artigo de \citeonline{Ezeobiejesi2017} utilizou uma pilha de Máquinas de Boltzmann Restritas (RBMs) para construir um modelo generativo de aprendizado de características. Para cada bloco da impressão digital, as características extraídas são repassadas a um classificador binário simples, que realiza a classificação.
...
Dada a importância desse estágio para a verificação de digitais, são realizadas diversas pesquisas por métodos de extração de minúcias das imagens.~\citeonline{TangMi1} trataram a extração de minúcias como um problema de detecção de objetos. Nesse método, uma CNN é utilizada para converter impressões digitais brutas em um mapa de pontuação de minúcias, com uma posição analisada a cada bloco de $16 \times 16$ pixels. As posições com pontuação acima de um determinado limiar são consideradas candidatas a minúcias. Em seguida, as regiões vizinhas são refinadas por uma segunda CNN que compartilha os mesmos níveis convolucionais, a qual também estima a orientação da minúcia.

\citeonline{TangMi2} propuseram uma Rede Neural Profunda, a \textit{FingerNet}, que combina conhecimento especializado do domínio de impressões digitais com treinamento de ponta a ponta para aumentar a precisão da extração de características. Especificamente, o modelo é inicialmente construído convertendo etapas tradicionais de extração de características (extração de orientação e detecção de minúcias) em camadas convolucionais com pesos fixos. Em seguida, essas camadas básicas são estendidas com novas camadas, e todos os pesos da rede são ajustados. Para treinar o modelo, é utilizada uma função de perda composta que aproveita rótulos fracos, fortes e verdadeiros relacionados à orientação, segmentação e minúcias.

\subsection{Verificação de impressões digitais}\label{subsubsec:verificacao_digitais}

Conforme explica~\citeonline{Maltoni20221} um algoritmo de verificação de impressões digitais compara duas imagens de digitais e retorna uma pontuação entre 0 e 1, em que 1 indica a maior similaridade possível, ou uma resposta binária. Poucas técnicas de verificação operam diretamente nas imagens em escala de cinza. A maioria deles opera em representações intermediárias, discutidas na Seção~\ref{subsec:representacao_de_digitais}.

O autor ainda afirma que as principais dificuldades na verificação de digitais estão relacionadas com a coleta das imagens, que estão sujeitas a ruídos causados por condições adversas na pele, rotação dos dedos ao encostar no sensor, parte do dedo pode ficar fora da área do sensor, etc. Isso porque, dos diversos algoritmos de verificação de digitais propostos na literatura, a grande maioria tem um bom desempenho quando utilizados em imagens de boa qualidade.

Segundo~\citeonline{Maltoni20221}, os algoritmos de verificação de digitais mais utilizadas são as que se baseiam nas características distintivas das minúcias, como a sua posição, tipo (bifurcação ou terminação de cume) e orientação. O autor explica que o problema pode ser visto como um reconhecimento de padrão de pontos, em que é necessário o emprego de técnicas de Visão Computacional, como o reconhecimento de objetos, sensoriamento remoto e detecção de movimento.

Dentre as principais abordagens, há o mapeamento de minúcias para representações de tamanho fixo, como foi feito nos trabalhos de~\citeonline{Engelsma2021} e de~\citeonline{Jin2016}, e também técnicas que utilizam tanto atributos globais como os padrões das linhas das digitais e atributos locais, como as minúcias, conforme apresentado em~\cite{ZHANG2019139}.
