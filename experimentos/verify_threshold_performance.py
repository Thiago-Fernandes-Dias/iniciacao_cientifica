...
for user in select(lw_g_c.frr, lambda x: x.user_id):
    ...
    if (g_bacc > u_bacc):
        cmu_users.append(user)
for user in cmu_users:
    print(f"O usuário {user} foi beneficiado com o ajuste global do limiar de corte")
print(f"No total, {len(cmu_users)} foram beneficiados.")
...
for user in select(lw_g_k.frr, lambda x: x.user_id):
    ...
    if (g_bacc > u_bacc):
        keyrecs_users.append(user)
for user in keyrecs_users:
    print(f"O usuário {user} foi beneficiado com o ajuste global do limiar de corte")
print(f"No total, {len(keyrecs_users)} foram beneficiados.")
