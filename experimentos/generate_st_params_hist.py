...
def gerar_histograma_threshold_horizontal(repo: Any, dataset_name: str):
    ...
    st_results_per_user = repo.read_results(f"Magalhães com HPO por usuário ({dataset_name})")
    ...
    avg_t_per_user = {
        ...
        for user_id, thresholds in ts_per_user.items() if thresholds
    }
    ...
    plt.barh(bin_centers, counts, height=(bin_edges[1]-bin_edges[0]), edgecolor='black', alpha=0.75)
