...
class OneClassExperimentRunnerImpl(OneClassExperimentRunner):
    ...
    def _calculate_predictions(self) -> None:
        ...
        for uk in self._dataset.user_keys():
            ...
            self._predictions_on_genuine_samples_map[uk] = self._estimator.predict(self._X_genuine_test[uk]).flatten().tolist()
            self._predictions_on_attacks_samples_map[uk] = self._estimator.predict(self._X_impostors_test[uk]).flatten().tolist()
