...
from lib.utils import cmu_first_session_split, keyrecs_split


class ExperimentExecutor:
    _name: str
    _results_repo: ResultsRepository
    _runner_factory: Callable[[Dataset, bool], OneClassExperimentRunner]

    def __init__(self, name: str, results_repo: ResultsRepository, runner_factory: Callable[[Dataset], OneClassExperimentRunner]):
        self._name = name
        self._results_repo = results_repo
        self._runner_factory = runner_factory
    
    def execute(self) -> None:
        cmu = CMUDataset(CMU_PATH, cmu_first_session_split)
